...
describe('OAuthList', () => {
  ...
  test('Should render specific oauth list', () => {
    ...
    const oauthItemDivOauthes = [...container.getElementsByClassName('VkIdSdk_oauth_item')].map((el) => el.getAttribute('data-oauth') );
  ...
  test('Should open auth with correct action', () => {
    ...
    expect(openFn).toBeCalledWith(expect.stringContaining(`action=${encodeURIComponent(btoa(action))}`), '_blank');
  });
});
